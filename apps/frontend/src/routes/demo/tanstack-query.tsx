  ...
  return (
    <main className="demo-page demo-center">
      <section className="demo-panel w-full max-w-2xl">
        ...
        {isLoading ? <p className="text-sm text-neutral-500">Loading tools...</p> : null}

        {isError ? <p className="text-sm text-red-600">Failed to load tools: {error.message}</p> : null}
        ...
        {!isLoading && !isError && tools.length > 0 ? (
          <ul className="mb-4 space-y-2">
            {tools.map((tool) => (
              <li className="demo-list-item" key={tool.id}>
                <span className="text-base font-medium">{tool.name}</span>
                <span className="ml-2 text-sm text-neutral-500">({tool.id})</span>
              </li>
            ))}
          </ul>
        ) : null}
